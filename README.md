...
The SVGs in `mark/` are generated by `src/marks.mjs`, and those in `social/` by
`src/social.mjs`, which imports the grid, the drawing and the tones rather than restating
any of them. `tokens.css` is generated by `src/tokens.mjs` from the same tones. Every
PNG, ICO and PDF is rendered from those SVGs inside a pinned Alpine image with librsvg
and ImageMagick. The photo ladder is resized from `photo/profile.png` in the same image.
